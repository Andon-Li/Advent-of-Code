...
def part1(puzzle_input):
    ...
    puzzle_matrix = ['.' + line + '.' for line in puzzle_input.split()]  # sentinel boundary
    puzzle_matrix.insert(0, '.'*142)
    puzzle_matrix.append('.'*142)
    for i in range(len(puzzle_matrix)):
        number_iter = re.finditer(r'\d+', puzzle_matrix[i])
        for number in number_iter:
            start_index = number.start()
            end_index = number.end()
            if (
                    any(char != '.' for char in puzzle_matrix[i-1][start_index-1:end_index+1]) or
                    any(char != '.' for char in puzzle_matrix[i+1][start_index-1:end_index+1]) or
                    puzzle_matrix[i][start_index-1] != '.' or
                    puzzle_matrix[i][end_index] != '.'):
...
def part2(puzzle_input):
    ...
    #  [number of adjacent numbers, product]
    adjacent_product_matrix = [[[0, 1] for _ in range(len(puzzle_matrix[0]))] for _ in range(len(puzzle_matrix))]
    ...
    for i in range(len(puzzle_matrix)):
        ...
        for number in number_iter:
            ...
            for j in range(start_index-1, end_index+1):
                adjacent_product_matrix[i - 1][j][1] *= value
                adjacent_product_matrix[i + 1][j][1] *= value

                adjacent_product_matrix[i + 1][j][0] += 1
                adjacent_product_matrix[i - 1][j][0] += 1

            adjacent_product_matrix[i][start_index - 1][1] *= value
            adjacent_product_matrix[i][end_index][1] *= value

            adjacent_product_matrix[i][start_index - 1][0] += 1
            adjacent_product_matrix[i][end_index][0] += 1

    for i in range(len(puzzle_matrix)):
        gear_iter = re.finditer(r'\*', puzzle_matrix[i])
        for gear in gear_iter:
            if adjacent_product_matrix[i][gear.start()][0] == 2:
                total += adjacent_product_matrix[i][gear.start()][1]
                print(adjacent_product_matrix[i][gear.start()])

    print(total)


puzzle_input = """...766.......821.547.....577......................................387.....................56..........446.793..........292..................
