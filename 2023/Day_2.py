...
def part1(puzzle_input):
    ...
    for game in puzzle_input.split('\n'):
        ...
        game_id = split_game[1]
        for i in range(3, len(split_game), 2):
            match split_game[i]:
                case 'red':
                    if int(split_game[i-1]) > 12:
                        valid_game = False
                case 'green':
                    if int(split_game[i-1]) > 13:
                        valid_game = False
                case 'blue':
                    if int(split_game[i-1]) > 14:
                        valid_game = False
        if valid_game:
            total += int(game_id)
    print(total)
...
def part2(puzzle_input):
    ...
    for game in puzzle_input.split('\n'):
        ...
        for i in range(3, len(split_game), 2):
            match split_game[i]:
                case 'red':
                    if int(split_game[i-1]) > min_red:
                        min_red = int(split_game[i-1])
                case 'green':
                    if int(split_game[i-1]) > min_green:
                        min_green = int(split_game[i-1])
                case 'blue':
                    if int(split_game[i-1]) > min_blue:
                        min_blue = int(split_game[i-1])
